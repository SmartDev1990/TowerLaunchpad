...
import { JSBI, Token, TokenAmount } from '@smartdev1990/tower-sdk'
...
const CreateSale: React.FC<CreateProps> = ({onDisagree, routeAddress}) => {
    ...
    const cakeToken: Token = tokens.cake
    const { account } = useWeb3React()
    const dispatch = useAppDispatch()
    const { toastError, toastSuccess } = useToast()
    const [pendingTx, setPendingTx] = useState(false)
    const [ agreed, setAgreed ] = useState<boolean>(false)
    const [ contributionType, setContributionType] = useState<ContributionType>(ContributionType.CMP)
    ...
    } = useTooltip("Your investors can contribute with $CMP or $TW", {
        placement: 'bottom',
    })
    ...
    const baseTokenDecimals = useMemo(() => {
        if (contributionType === ContributionType.CMP) {
            return 18
        }

        if (contributionType === ContributionType.TW) {
            return cakeToken.decimals
        }

        return customToken?.decimals ?? 18
    }, [contributionType, cakeToken, customToken])
    ...
    const handleCreate = useCallback(async () => {
        ...
        try {
            ...
            if (contributionType === ContributionType.TW) {
                baseTokenAddress = cakeToken.address
                    ...
                    <Flex flexDirection={["column", "column", "column", "row"]} maxWidth="960px" width="100%">
                        <Flex flexDirection="column" flex="1" order={[1, 1, 1, 0]}>
                            <InputWrap>
                                    ...
                                    <Flex>
                                        <Flex flex="1">
                                            <RadioWithText
                                                checked={contributionType === ContributionType.CMP}
                                                onClick={() => setContributionType(ContributionType.CMP)}
                                                text={t('CMP')}
                                                />
                                        </Flex>
                                        <Flex flex="1">
                                            <RadioWithText
                                                checked={contributionType === ContributionType.TW}
                                                onClick={() => setContributionType(ContributionType.TW)}
                                                text={t('TW')}
                                                />
                                        </Flex>
                                        {/* <Flex flex="1">
                        ...
                        <Flex flexDirection="column" order={[0, 0, 0, 1]} margin={["0 0px 24px 0px", "0px 0px 24px 0px", "0px 0px 24px 0px", "0px 0px 0px 48px"]} maxWidth={["100%", "100%", "100%", "50%"]}>
                            ...
                            <StyledList>
                                ...
                                <li>
                                {t('Deploy Fee: %amount% %currency%', {amount: getFullDisplayBalance(deployFeeNumber), currency: 'CMP'})}
                                </li>
